    ...
    return (
      <div className="bg-black text-white px-3.5 py-2.5 rounded-none border border-black/80 shadow-md text-xs font-sans">
        ...
        <p className="font-bold text-white text-sm">
          Net Income: {formatCurrencyTick(val)}
        </p>
      </div>
    );
  }
...
const NetIncomeChart = ({ periods = [], source = 'FMP Income Statements', className = '' }) => {
  const chartData = React.useMemo(() => {
    ...
    const valid = periods
      .filter((p) => p && p.netIncome !== undefined && p.netIncome !== null)
      .map((p, i) => ({
    ...
    return (
      <div className={`bg-white rounded-none border border-black/20 p-6 shadow-xs ${className}`}>
        <div className="flex justify-between items-center mb-3">
          <h3 className="font-sans font-bold text-lg text-black">Net Income Trend</h3>
        </div>
        <div className="h-56 flex items-center justify-center bg-gray-50 rounded-none text-gray-500 font-sans">
          Net income historical trend unavailable.
        </div>
      </div>
    );
  }
