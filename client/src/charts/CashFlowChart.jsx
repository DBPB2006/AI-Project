    ...
    return (
      <div className="bg-black text-white px-3.5 py-2.5 rounded-none border border-black/80 shadow-md text-xs font-sans">
        ...
        <p className="font-bold text-white text-sm">
          Free Cash Flow: {formatCurrencyTick(val)}
        </p>
      </div>
    );
  }
...
const CashFlowChart = ({ periods = [], source = 'FMP Cash Flow', className = '' }) => {
  const chartData = React.useMemo(() => {
    ...
    const valid = periods
      .filter((p) => p && (p.cashFlow !== undefined || p.freeCashFlow !== undefined) && (p.cashFlow !== null || p.freeCashFlow !== null))
      .map((p, i) => ({
        period: p.period || p.calendarYear || p.date || (periods.length === 1 ? 'Reported' : `Period ${i + 1}`),
        cashFlow: Number(p.cashFlow !== undefined ? p.cashFlow : p.freeCashFlow)
      }));
    ...
    return (
      <div className={`bg-white rounded-none border border-black/20 p-6 shadow-xs ${className}`}>
        <div className="flex justify-between items-center mb-3">
          <h3 className="font-sans font-bold text-lg text-black">Free Cash Flow Trend</h3>
        </div>
        <div className="h-56 flex items-center justify-center bg-gray-50 rounded-none text-gray-500 font-sans">
          Free cash flow trend data unavailable.
        </div>
      </div>
    );
  }
      ...
      <div className="h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 10, right: 15, left: 15, bottom: 5 }}>
            ...
            <Bar dataKey="cashFlow" fill="#0891b2" radius={[0, 0, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
